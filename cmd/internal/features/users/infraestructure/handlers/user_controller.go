...
import (
	usecases "daedalus-engine-go/cmd/internal/features/users/application/use_cases"
	requests "daedalus-engine-go/cmd/internal/features/users/domain/dtos/requests"
	domain "daedalus-engine-go/cmd/internal/features/users/domain/entities"
	mappers "daedalus-engine-go/cmd/internal/features/users/domain/mappers"
...
func (ctr *UserController) ObtenerUsuario(c *gin.Context) {
	...
	if email == "" {
		...
		validationErr := baseErrors.NewValidationError("REQUEST_002", "Email es requerido")
		statusCode, errorResponse := httpErrors.MapErrorToHttp(validationErr)
		c.JSON(statusCode, errorResponse)
		return
	}
...
func (ctr *UserController) EliminarUsuario(c *gin.Context) {
	...
	if id == "" {
		...
		validationErr := baseErrors.NewValidationError("REQUEST_003", "Id es requerido")
		statusCode, errorResponse := httpErrors.MapErrorToHttp(validationErr)
		c.JSON(statusCode, errorResponse)
		return
	}
